...
require(
    ...
    and "std::shared_ptr<const result_persistence::ChartResultAttempt> attempt" in result_header
...
for exclusion in (
    ...
    require(exclusion in policy_body, f"central result capture policy is missing {exclusion}")
require(
...
require(
    "receipt.reset()" in schedule_body
    and "resultPersistenceOptions.attempt.reset()" in schedule_body,
    "attempt construction failure must remain sanitized and non-durable",
)
require(
    "saveStateUserMessage(" in schedule_body,
    "attempt construction failure must reuse coordinator-owned user copy",
)
require(
...
for sensitive in ("recordedReplay", "BmsPath", "chart->Meta.Path", "attempt->replay"):
    for log_call in re.findall(r"SDL_Log[^;]*;", schedule_body, re.DOTALL):
        require(sensitive not in log_call, f"persistence logging must not expose {sensitive}")
...
require(
    ...
    and "SaveCourseScore(" in result_source[course_score_span[0] : course_score_span[1]],
    "course score persistence must remain in its course-only method",
)
require(
    course_replay_span is not None
    and "SaveCourseReplay(" in result_source[course_replay_span[0] : course_replay_span[1]],
    "course replay persistence must remain in its course-only method",
)
require(
    "SaveCourseScore(" not in masked_source and "SaveCourseReplay(" not in masked_source,
...
require(
    "receipt->replayId" in receipt_body
...
require(
    ...
    and "persistenceOptions.outcome.receipt" in previous_body
...
require(
    ordered(previous_body, "beforeCreatedAt", "excludeAttemptId", "LoadBestScore("),
...
require(
    ...
    and "scripts/check_result_persistence_flow.sh" in cmake,
    "the result persistence flow audit must be registered with CTest",
)
