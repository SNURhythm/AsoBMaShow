...
| Medium | The Java-pattern adapter used Java 19+ ASCII word-boundary behavior even though pinned Beatoraja requires Java 17, where the default boundary predicate includes Unicode letters/digits and contextually attached non-spacing marks. | Implemented Java 17's separate legacy and `UNICODE_CHARACTER_CLASS` boundary predicates, including its asymmetric UTF-16 treatment before and after supplementary marks; pinned macOS CI to Zulu 17; and passed both native backends against alphabetic-symbol, connector-punctuation, join-control, number-category, combining-mark, `\b`, and `\B` oracle cases. |
| Medium | The hardened iOS dependency graph left Firebase App Distribution on its legacy raw-file upload path, which the newer Faraday/Net::HTTP adapter rejected because the stream had no content length. | Pinned Firebase App Distribution plugin 1.0.0 and its current generated API clients, replacing the raw stream hack with Firebase's supported resumable upload API; a release policy prevents downgrading to the incompatible client. |
...
| Low | New release-contract test wiring relied on transitive curl headers, created the curl test on iOS where that backend is absent, and registered Lua execution tests even when their native targets were disabled. | Linked the curl contract to its imported dependency, limited it to platforms with the curl backend, registered execution-dependent tests only when their targets exist, and confirmed that a fresh Lua-disabled CTest discovery contains no ledger execution tests. |
...
| Android policy tests | 12/12 release workflow tests and 28/28 cross-platform contract tests passed. A native Gradle configuration probe confirmed that a standalone-style `ANDROID_NDK_HOME` is passed through as AGP's NDK path and CMake's Android NDK; an existing-APK upload probe confirmed that invalid build-toolchain settings are ignored. |
...
| iOS policy tests | 49/49 setup, 19/19 workflow, 16/16 artifact-audit, and 3/3 documentation tests passed. |
...
The last portability review added regressions for Java 17's leading
supplementary-mark boundary behavior, proved that existing-APK uploads bypass
