...
- `src/skin/package/SkinPackageCatalog.{h,cpp}`: versioned private metadata, journal recovery, validation records, and activation pointers.
- `src/skin/package/SkinPackageStore.{h,cpp}`: archive/folder prepare, visible publication, manual rescan, activation, retention, and garbage collection.
...
- Produces: typed prepare/publish/scan/GC results; one app-owned serialized `SkinPackageOperationService` for every filesystem/validation operation and discarded staging cleanup; store-level main-thread begin/poll operations; one scene-independent app-owned `SkinCommitCoordinator` for activation and profile-only skin commits; and `AcquireActivationResult acquireValidatedActivation(profileId, entry, configurationDigest)`. Validation never mutates profile/catalog state; the main-thread commit serializes the durable profile update before the activation CAS.
...
- [ ] **Step 9: Run the GREEN check**
  ...
  Expected GREEN: fault injection at every boundary recovers to exactly the old or new whole package, never a merge, and no detached or deleted profile leaves a live ticket/lease/catalog activation.
- [ ] **Step 10: Commit the task**
