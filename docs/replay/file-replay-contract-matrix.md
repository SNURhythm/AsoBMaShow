...
| Shared result/setup domains | `result_contract` accepts every positive gameplay key count and owns chart identity agreement, gauge/clear domains, and score arithmetic; `ReplayKeyMode` narrows only BRD setup to stock Beatoraja layouts | none | active for replay setup, modern results, recall, and provider-neutral IR snapshots without making a replay file mandatory for result history; providers narrow their own upload support |
...
| Summary-only schema transition | schema v14 `ReplayRepositoryLegacyMigration` header decoders, exact-schema inspection, count/reference verification, and one SQLite transaction | none; migration performs no file I/O and never reads playback-detail tables | active; chart/course headers and independently durable IR evidence survive while replay/event/touch/lane-cover/course-stage and pending legacy write tables are dropped atomically |
| Legacy Records projection | `LegacyResultSummary`, bounded repository summary reads, `ResultRecordSummary`, and `replay::capabilitiesFor` | none | active; nullable/partial facts remain honest Records history with no detailed result, replay, file, video, ghost, G-Battle, retry, or new IR capability |
| Modern saved-result IR | one repository read snapshot over modern result, stored `IrSubmissionSnapshot`, provider-scoped receipt, and outbox; `IrUploadCandidates` owns cross-row agreement | none | active; manual uploads and reconciliation use modern result IDs and canonical attempt IDs, never legacy rows or replay materialization |
| Summary-aware profile transfer | pre-migration SQLite backup comparison, schema-owner migration, exact v14 validation, and agreed modern replay inventory | legacy summaries require no file; modern files follow the inventory policy above | active for profile duplication and archive import/export; complete and partial summary rows survive without recreating legacy playback tables |
