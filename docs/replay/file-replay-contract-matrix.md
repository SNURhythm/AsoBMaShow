...
| Shared result/setup domains | `result_contract` result-capable key modes, chart identity agreement, gauge/clear domains, and score arithmetic; `ReplayKeyMode` narrows BRD setup to stock Beatoraja layouts | none | active for replay setup, modern results, recall, and IR without making a replay file mandatory for result history |
