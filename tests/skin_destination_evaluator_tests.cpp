...
void testObjectAccelerationAndPinnedOffsetAlphaBranches() {
  ...
  const auto laterAcceleration = evaluateSkinDestinationAuthored(latched, inputs(350'000, 0));
  expect(laterAcceleration.geometry && near(laterAcceleration.geometry->rect.x, 11.25),
  ...
  expect(steppedMidpoint.geometry && near(steppedMidpoint.geometry->rgba[3], 0.0),
...
void testFractionalOffsetAndClipSuppression() {
  ...
  const std::array<ConfigOffset, 1> odd = {
      {{.x = 3, .y = -5, .w = 5, .h = -3}}};
  const auto halfShift = evaluateSkinDestinationAuthored(body, inputs(100'000, 0, {}, odd));
  expect(halfShift.geometry && near(halfShift.geometry->rect.x, 10.5) &&
         ...
         "odd positive and negative offsets preserve Java floating half-unit shifts");

  const std::array<ConfigOffset, 1> zeroWidth = {{{.w = -60}}};
  const auto noWidthClip = evaluateSkinDestinationAuthored(
      body, inputs(100'000, 0, {}, zeroWidth));
...
void testSourceRegionStretchAndProjection() {
  ...
  const std::array<Expected, 11> expected = {{{10.0, 20.0, 90.0, 30.0, 101, 51, 61, 41},
  ...
  for (int id = 0; id <= 10; ++id) {
    ...
    expect(near(projected.vertices[0][0], want.x) && near(projected.vertices[0][1], 100.0 - want.y) &&
    ...
    expect(near(projected.normalizedUvs[0][0], static_cast<double>(want.regionX) / 400.0) &&
               near(projected.normalizedUvs[0][1], static_cast<double>(want.regionY) / 300.0) &&
               near(projected.normalizedUvs[2][0], static_cast<double>(want.regionX + want.regionWidth) / 400.0) &&
               near(projected.normalizedUvs[2][1], static_cast<double>(want.regionY + want.regionHeight) / 300.0),
  ...
  expect(near(trimmed.normalizedUvs[0][1], 61.0 / 300.0) && near(trimmed.normalizedUvs[2][1], 81.0 / 300.0),
  ...
  expect(near(noResize.vertices[0][0], 24.5) && near(noResize.vertices[2][0], 85.5),
  ...
  const auto bothTrimmed = projectSkinDestinationToUi(geometry, source, viewport);
  ...
  expect(near(rotated.vertices[0][0], 75.5) && near(rotated.vertices[0][1], 95.5) &&
             near(rotated.vertices[2][0], 34.5) && near(rotated.vertices[2][1], 34.5),
         "rotation occurs after stretch adjustment");

  const auto invalidTexture = projectSkinDestinationToUi(
      geometry, {.textureWidth = 0, .textureHeight = 300, .region = source.region}, viewport);
  const auto invalidRegion = projectSkinDestinationToUi(
      geometry, {.textureWidth = 400, .textureHeight = 300, .region = {.x = 1, .y = 1, .w = 0, .h = 1}}, viewport);
  expect(invalidTexture.vertices == UiDestinationGeometry{}.vertices &&
  ...
  geometry.clip = SkinAuthoredRect{.x = 10.0, .y = 20.0, .width = -1.0, .height = 4.0};
  const auto negativeClip = projectSkinDestinationToUi(geometry, source, viewport);
  expect(!negativeClip.clip, "projection never resurrects a negative clip with absolute dimensions");
}
