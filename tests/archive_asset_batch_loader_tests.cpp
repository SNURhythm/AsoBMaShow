...
void testPartialFailureRetriesOnlyMissingEntries(bool cancelAfterDelivery) {
  ...
  auto streaming = [&](const auto &, const auto &missing, auto onFile, auto *, auto) {
    ++fallbackCalls;
...
void testSerialOnlyReaderConsumesEveryEntryOnce(std::size_t workers) {
  ...
  auto streaming = [&](const auto &, const auto &requested, auto onFile, auto *, auto) {
    ++serialCalls;
...
void testInlineConsumptionRejectsIncompleteDeliveryAndConsumerFailure() {
  ...
  for (const bool reject : {false, true}) {
    unsigned consumed = 0;
    auto streaming = [&](const auto &, const auto &, auto onFile, auto *, auto checkpoint) {
