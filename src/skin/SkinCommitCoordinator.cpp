...
constexpr std::size_t kMaxRevalidationRequests = 128;
...
void appendBounded(std::deque<Completion> &queue, Completion completion) {
  if (queue.size() == kMaxDeliveryRecordsPerClient) {
    queue.pop_front();
  }
...
struct SkinCommitCoordinator::Impl {
  ...
  bool accepts(SkinActivationClientId client,
    ...
    return !stopping && client != 0 && clients.contains(client) &&
  ...
  void recordActivation(SkinActivationClientId client,
    ...
    appendBounded(activationCompletions[client],
  ...
  void recordProfile(SkinActivationClientId client,
    ...
    appendBounded(profileCompletions[client],
                  SkinProfileCommitCompletion{.client = client,
                                              .ticket = coordinatorTicket,
                                              .result = std::move(result)});
  }

  void recordRevalidation(const CommitActivationResult &result) {
    if (stopping ||
        result.disposition !=
    ...
    if (revalidationRequests.size() == kMaxRevalidationRequests) {
      revalidationRequests.pop_front();
    }
    revalidationRequests.push_back(*result.profileSnapshot);
  }
  ...
  void pollOnce() {
    for (auto iterator = activations.begin(); iterator != activations.end();) {
      ...
      recordRevalidation(result);
      recordActivation(client, coordinatorTicket, std::move(result));
    ...
    for (auto iterator = profiles.begin(); iterator != profiles.end();) {
      ...
      try {
        ...
        // A throwing owner has not supplied a terminal result, so it cannot be
        // acknowledged or forgotten safely.
        ++iterator;
        continue;
      }
      ...
      recordProfile(client, coordinatorTicket, std::move(result));
  ...
  bool stopping = false;
  bool stopped = false;
  std::set<SkinActivationClientId> clients;
  std::map<std::uint64_t, ActivationTransaction> activations;
  std::map<std::uint64_t, ProfileTransaction> profiles;
  std::map<SkinActivationClientId, std::deque<SkinActivationCompletion>>
      activationCompletions;
  std::map<SkinActivationClientId, std::deque<SkinProfileCommitCompletion>>
      profileCompletions;
  std::deque<VersionedSkinProfileSettings> revalidationRequests;
};
...
SkinCommitCoordinator::takeCompletions(SkinActivationClientId client) {
  ...
  if (!impl_->onMainThread() || !impl_->clients.contains(client)) {
    return result;
  }
  auto found = impl_->activationCompletions.find(client);
  if (found == impl_->activationCompletions.end()) {
    return result;
  }
  result.reserve(found->second.size());
  while (!found->second.empty()) {
    result.push_back(std::move(found->second.front()));
    found->second.pop_front();
  }
  impl_->activationCompletions.erase(found);
  return result;
}
...
SkinCommitCoordinator::takeProfileCompletions(SkinActivationClientId client) {
  ...
  auto found = impl_->profileCompletions.find(client);
  if (found == impl_->profileCompletions.end()) {
    return result;
  }
  result.reserve(found->second.size());
  while (!found->second.empty()) {
  ...
  impl_->profileCompletions.erase(found);
  return result;
}
...
void SkinCommitCoordinator::detachClient(
  ...
  try {
    ...
    impl_->activationCompletions.erase(client);
    impl_->profileCompletions.erase(client);
  } catch (...) {
  }
}
...
SkinCommitCoordinator::takeRevalidationRequests() {
  ...
  result.reserve(impl_->revalidationRequests.size());
  while (!impl_->revalidationRequests.empty()) {
    result.push_back(std::move(impl_->revalidationRequests.front()));
    impl_->revalidationRequests.pop_front();
  }
  return result;
}
...
void SkinCommitCoordinator::finishProfileMutation(
  ...
  try {
    ...
    if (mutationSucceeded) {
      impl_->store.removeProfileActivations(profile);
    }
...
void SkinCommitCoordinator::shutdown() noexcept {
  try {
    ...
    impl_->stopping = true;
    impl_->clients.clear();
    impl_->activationCompletions.clear();
    impl_->profileCompletions.clear();
    impl_->revalidationRequests.clear();
