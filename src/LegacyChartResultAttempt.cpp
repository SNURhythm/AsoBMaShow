...
using result_persistence::ChartJudgementTiming;
using result_persistence::ChartScoreWrite;
namespace {
...
std::optional<LegacyChartResultAttempt> rejected(std::string_view invariant,
...
std::optional<LegacyChartResultAttempt> makeLegacyChartResultAttempt(
  ...
  if (!legacy_result_persistence::hasProjectableChartIdentity(score)) {
  ...
  const std::string fingerprint = legacyPayloadFingerprint(replay, score);
  ...
  return LegacyChartResultAttempt{.attemptId = std::move(attemptId),
...
std::string legacyPayloadFingerprint(const ReplayData &replay,
