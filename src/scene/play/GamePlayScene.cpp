...
#include "CourseReplayStageStartup.h"
...
bool prepareRetryChart(const bms_parser::ChartMeta &meta,
  ...
  const auto replayFacts = replay::captureAuthoredReplayChartFacts(*retryChart);
  ...
  retryOptions.hasUndefinedLongNotes = replayFacts.hasUndefinedLongNotes;
  ...
  if (sourceOptions.playOption.has_value() &&
      !play_options::applyPlayOptionModifier(
          *retryChart, *sourceOptions.playOption, std::nullopt, 0,
          retryOptions.playOption, retryOptions.playOptionSeed, "retry")) {
    return false;
  }
...
struct GamePlayScene::RealtimeGameplaySession {
  ...
  emitPhysicalInput(void *context,
    ...
    if (transition.type ==
      ...
      auto command = transition.command;
      command.steadyTimestampMicros = transition.steadyTimestampMicros;
      if (!session.inputCommands.tryPush(command)) {
...
void GamePlayScene::stopRealtimeGameplayAuthority(bool transferReplay) {
  ...
  if (transferReplay) {
    ...
    const auto workerInput = session.worker->copyAcceptedReplayInputAfterStop();
...
void GamePlayScene::init() {
  ...
  if (!isReplayPlayback() && !options.autoPlay) {
    ...
    ownedInputHandler = std::make_unique<RhythmInputHandler>(
        ...
        [this](const LogicalGameplayInputAdapter::AppliedTransition &applied) {
          captureReplayAppliedTransition(applied);
        });
...
void GamePlayScene::handleLogicalInputCommand(
  ...
  if (transition.action.kind == input::LogicalActionKind::Start ||
    ...
    captureReplayControl(
        transition.steadyTimestampMicros,
        {.kind = transition.action.kind == input::LogicalActionKind::Start
...
void GamePlayScene::configurePacemakerTarget() {
  ...
  if (isReplayPlayback()) {
    ...
    const ReplayComparisonQuery replayComparison = replayComparisonQueryFor(
        options.replayResultContext.has_value() ? &*options.replayResultContext
                                                : nullptr);
...
bool GamePlayScene::startCourseReplayChartAtCurrentIndex() {
  ...
  if (session == nullptr ||
    ...
    if (!outcome.prepared()) {
      SDL_Log("Course replay stage start failed: %s",
...
void GamePlayScene::beginReplayRecording() {
  ...
  replayInputCapture = std::make_unique<replay::ReplayInputCaptureBuffer>();
...
void GamePlayScene::finishReplayRecording() {
  if (!shouldRecordReplay() || state == nullptr || rawReplayFinished) {
    return;
  }
...
void GamePlayScene::publishPracticeGhost() {
  ...
  const JudgedPlaybackData *completedReplay =
      practice::completedAttemptForGhost(options.practiceSession.get(),
                                         recordedReplay,
    ...
    if (!resultPersistenceAttemptCreationTried) {
      ...
      if (rawReplayCaptureFailed) {
        ...
        result = result_persistence::capturePersistedChartResult(
            ...
            scoreLongNoteModeForClearLamp(chart->Meta), playedAtUnixMillis,
            resultDiagnostic);
        if (result.has_value()) {
    ...
    for (size_t j = isFirstMeasure ? state->passedTimelineCount : 0;
      ...
      if (timeline->Timing < poorCutoff) {
        ...
        for (const auto &note : timeline->LandmineNotes) {
          ...
          if (!legacyReplayPlayback && laneIsPressed(lanePressed, note->Lane)) {
