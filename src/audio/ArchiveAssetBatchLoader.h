...
bool ConsumeArchiveAssetBatch(
  ...
  if (innerPaths.size() == 1 || workerCount <= 1 || maximumBytes == 1) {
    ...
    const bool read = readStreaming(archivePath, innerPaths, onFile,
  ...
  // Each stage uses a scheduling budget and admits one oversized asset when idle.
  ...
  bool read = false;
  if (extractWorkers > 1) {
    read = readConcurrently(
        archivePath, innerPaths, onFile, extractWorkers,
  ...
  if ((!read || acceptedPaths.size() != innerPaths.size()) &&
    ...
    read = missing.empty() ||
           readStreaming(archivePath, missing, onFile,
                         errorMessage, continueReading);
  }
